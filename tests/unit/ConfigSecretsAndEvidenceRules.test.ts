...
describe('SEC-016 InboundAuthenticationEvidenceRule', () => {
  ...
  it('is not satisfied by an outbound connector using OAuth', async () => {
    // An outbound oauth grant authenticates a call this application makes; it is
    // not a control protecting an inbound listener, so it must not suppress the
    // finding. Exercised through a real scan so the pre-scan inventory decides.
    ...
    write(
      ...
      [
        ...
        '  <http:request-config name="SF">',
        '    <http:request-connection host="sf.example.com" protocol="HTTPS">',
        '      <http:authentication>',
        '        <oauth:client-credentials-grant-type clientId="a" tokenUrl="c"/>',
        '      </http:authentication>',
    ...
    const report = await new LintEngine({
      rules: [new InboundAuthenticationEvidenceRule()],
    }).scan(projectRoot);

    const found = report.files.flatMap((file) => file.issues).filter((i) => i.ruleId === 'SEC-016');
