...
export class ErrorResponseStructureRule extends BaseRule {
  ...
  validate(doc: Document, context: ValidationContext): Issue[] {
    ...
    // project context is absent (standalone scan) the check is skipped.
   ...
   *
   * Covers the core set-payload, a transform with an external DWL resource, and
   * a raise-error that hands responsibility to an outer handler.
   */
  private producesPayload(block: Node): boolean {
    ...
    for (const producer of payloadProducers) {
      if (this.exists(`.//*[local-name()="${producer}"]`, block)) {
        return true;
      }
    }
